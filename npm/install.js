...
async function extractBinary(archiveBuf, binFile, fmt, destPath) {
  ...
  try {
    if (fmt === 'tar.gz') {
      ...
      // Extract to a temp dir to avoid overwriting the bin/bcdock JS shim,
      // then rename to destPath (e.g. bcdock -> bcdock-native).
      const tmpDir = destPath + '.extractdir';
      ...
      try {
        ...
        const extracted = path.join(tmpDir, binFile);
