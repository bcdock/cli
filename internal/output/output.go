...
// the decoded API record `raw` for json. Used by list/get commands so callers
// of `-o json` get the underlying record (full GUIDs, every field OF THE
// DECODED STRUCT, nested fields preserved) instead of the truncated row struct.
//
// ⚠️ `raw` is NOT the server's bytes. ALL 16 non-test call sites pass a value
// they already decoded - checked, including the four that are not a bare
// identifier (`*match`, `resp.Snapshots`, `resp.Grants`) - so a field the API
// sent that the struct does not declare was dropped on the way IN and cannot
// reappear here. `-o json` is not
// an escape hatch from a lossy struct - it is the same loss, rendered.
// This sentence used to read "every field", which is why OPS-110 sat three
// fields deep in adminImage unnoticed: the table looked lossy, `-o json`
// looked complete, and the conclusion was that the API never sent them.
//
