...
type environment struct {
	...
	// SEC-039: `username` stays on the detail response - it is tier-1 standing-visible,
	// not secret material. `password` and `webServiceAccessKey` do NOT: they come only
	// from the audited reveal, decoded into environmentCredentials. Removed from this
	// struct rather than left to decode as nil, so a consumer that still wants them
	// fails to compile instead of silently getting nothing.
