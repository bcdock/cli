...
func envPublishHarness(t *testing.T, devHandler http.HandlerFunc) (apiURL string, devCalls *int32) {
	...
	// SEC-039: the secrets no longer ride on the detail response; publish reveals them.
	mux.HandleFunc("/api/v1/environments/"+publishEnvID+"/credentials", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]any{
			"environmentId":       publishEnvID,
			"username":            "admin",
			"password":            "P@ssw0rd!",
			"webServiceAccessKey": "wsk-pub",
		})
	})
