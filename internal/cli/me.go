...
Examples:
  bcdock me export                          # fire-and-forget; check later with 'me show'
  bcdock me export --wait                   # poll until ready, then print download URL
  bcdock me export --wait --out export.zip  # poll, then download the ZIP locally
  bcdock me export -o json                  # raw response shape, no human formatting

Exit codes: 0 ok · 3 auth · 4 rate-limited · 124 timeout (--wait only) · 1 other.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		r := GetResolved(cmd)
		if r.Token == "" {
			return fmt.Errorf("not authenticated - run 'bcdock auth set-token <token>' or set BCDOCK_TOKEN")
...
Examples:
  ...
  bcdock me delete --confirm you@example.com -o json    # for scripts
