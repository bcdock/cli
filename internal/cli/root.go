...
// rootHelpTemplate is the help template for the root command only. It renders
// command groups in display order and appends the exit-codes table, which is
// not in Long (Long is shown in parent command listings where exit codes are
// noise). Subcommands inherit this template but the groups and exit-codes
// sections are guarded by {{if not .HasParent}}.
const rootHelpTemplate = `{{.Long}}

Usage:
  {{.UseLine}}{{if .HasAvailableSubCommands}} [command]{{end}}{{if .HasExample}}

Examples:
{{.Example}}{{end}}{{if and .HasAvailableSubCommands (not .HasParent) .Groups}}{{range $group := .Groups}}{{if ne $group.ID "helpTopics"}}

{{$group.Title}}:
{{range $.Commands}}{{if and (eq .GroupID $group.ID) .IsAvailableCommand}}  {{rpad .Name .NamePadding}} {{.Short}}
{{end}}{{end}}{{end}}{{end}}{{else if .HasAvailableSubCommands}}
...
{{range .Commands}}{{if .IsAdditionalHelpTopicCommand}}  {{rpad .Name .NamePadding}} {{.Short}}
{{end}}{{end}}{{end}}{{if not .HasParent}}
...
Exit codes:
  ...
  124  timeout{{end}}{{if .HasAvailableSubCommands}}
...
var RootCmd = &cobra.Command{
	...
	Long: `Welcome to BC[)OCK! Managed Business Central sandboxes in cloud.
...
func init() {
	RootCmd.AddGroup(
		...
		&cobra.Group{ID: "account", Title: "Account & billing"},
		&cobra.Group{ID: "discovery", Title: "Discovery"},
		&cobra.Group{ID: "utility", Title: "Utility"},
		&cobra.Group{ID: "helpTopics", Title: "Help Topics:"},
	)
