...
FAST=yes means a pre-built image is ready (~2 min provisioning vs ~15 min).

Examples:
  bcdock artifacts list --region australiaeast
  bcdock artifacts list --region australiaeast --country au
  bcdock artifacts list --region australiaeast --type sandbox
...
func init() {
	...
	artifactsListCmd.Flags().Bool("fast-only", false, "Only versions with pre-built images (~2 min provisioning)")
