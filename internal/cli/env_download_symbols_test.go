...
func envSymbolsHarness(t *testing.T, devHandler http.HandlerFunc) (apiURL string, calls *int32) {
	...
	// SEC-039: the secrets no longer ride on the detail response; download-symbols reveals them.
	mux.HandleFunc("/api/v1/environments/"+symbolsEnvID+"/credentials", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]any{
			"environmentId":       symbolsEnvID,
			"username":            "admin",
			"password":            "P@ssw0rd!",
			"webServiceAccessKey": "wsk-sym",
		})
	})
