...
type waitlistFullRequest struct {
	...
	// SEC-012: the CLI can't clear a browser Turnstile challenge, so it requests the
	// deferred flow - the server creates a pending entry and returns a confirm URL the
	// human opens to clear Turnstile in a browser. Always true from the CLI.
...
func runJoinWaitlist(cmd *cobra.Command, _ []string) error {
	...
	if resp.ConfirmUrl != "" {
		// SEC-012: deferred flow - the human clears Turnstile in a browser to confirm.
		r.Printer.Info("")
		r.Printer.Info("One last step - open this link in a browser to confirm you're human:")
		r.Printer.Info("  %s", resp.ConfirmUrl)
		r.Printer.Info("")
	}
	r.Printer.Info("Once an admin reviews your entry you'll receive an invite code by email.")
	r.Printer.Info("No email? Check your Junk or Spam folder.") // #576: outlook.com can junk it (#402)
...
func runSignup(cmd *cobra.Command, _ []string) error {
	...
	if err := r.Client.Do(cmd.Context(), http.MethodPost, "/api/v1/public/validate-invite-code",
		map[string]string{"code": inviteCode, "email": email}, &validation); err != nil {
		return err
	}
	if !validation.Valid {
