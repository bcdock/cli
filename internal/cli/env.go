...
Exit codes:
  ...
  bcdock env create --name my-env --version 25.5 --country au --region westus2 --wait`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// DEV-024: a manifest fills anything the caller did not set explicitly.
		//
		// Precedence is explicit flag > manifest > built-in default, and "explicit"
		// means Changed(), never the value: --type defaults to "sandbox" and
		// --multi-tenant to true, so testing the value would let a flag the user
		// never typed beat the manifest.
		//
		// LoadManifest validates fully, and it runs HERE - before the first API
		// call - so a bad manifest creates nothing.
		...
		if manifestPath != "" {
			...
			if !cmd.Flags().Changed("version") && manifest.BCVersion != "" {
				version = manifest.BCVersion
			}
			if !cmd.Flags().Changed("country") && manifest.Country != "" {
				country = manifest.Country
			}
			if !cmd.Flags().Changed("name") && manifest.Name != "" {
				name = manifest.Name
			}
			if !cmd.Flags().Changed("region") && manifest.Region != "" {
				region = manifest.Region
			}
			if !cmd.Flags().Changed("type") && manifest.ArtifactType != "" {
				envType = manifest.ArtifactType
			}
			if !cmd.Flags().Changed("multi-tenant") && manifest.MultiTenant != nil {
				multiTenant = *manifest.MultiTenant
			}
			// Apps can only be published into a Running environment, so --manifest
			// implies --wait. Returning at "creating" would report success on an
			// environment whose apps were never published.
			//
			// An EXPLICIT --wait=false is a contradiction, not a preference to
			// override: the caller asked for two things that cannot both happen.
			// Silently winning the argument would publish nothing and exit 0
			// (Ravi, #280). Every other field here asks Changed(); so does this one.
			if cmd.Flags().Changed("wait") && !wait {
				return fmt.Errorf("--wait=false cannot be combined with --manifest: apps publish " +
					"into a running environment, so the manifest path must wait. Drop --wait=false, " +
					"or drop --manifest and publish with 'bcdock env publish'")
			}
			wait = true
		}
		...
		// DEV-024: the environment is Running, so publish the manifest's apps in
		// manifest order.
		if manifest != nil {
			insecure, _ := cmd.Flags().GetBool("insecure")
			if perr := publishManifestApps(cmd.Context(), r.Printer, r.Client, created.ID, manifest, multiTenant, insecure); perr != nil {
				// Print the environment BEFORE returning the error. The environment
				// is real, running and billable, and its shortId is the only handle
				// the caller has for re-publishing or deleting it - losing that in
				// an error path would be worse than the publish failure itself.
				_ = printEnvResult(r.Printer, *env)
				return perr
			}
		}
...
Exit codes:
  ...
  bcdock env download-symbols my-env --force --tenant default`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// SEC-039: one reveal per invocation. The detail read above no longer carries
		// credentials, so the secrets are fetched here, deliberately, and audited.
		creds, err := revealEnvCredentials(cmd.Context(), r.Client, id)
		if err != nil {
			return err
		}
		bcUser, bcPass, err := requireBcAdmin(creds, envArg, env.Status)
		if err != nil {
			return err
		}
		...
		return downloadSymbols(cmd.Context(), r.Printer, *env.DevEndpointUrl,
			bcUser, bcPass, tenant, targets, outDir, force, timeout, insecure)
	},
}
...
Exit codes:
  ...
  bcdock env publish my-env app.app --tenant default --timeout 20m`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// SEC-039: one reveal per invocation, audited. The detail read above carries no
		...
		return publishApp(cmd.Context(), r.Printer, publishURL,
			bcUser, bcPass, filepath.Base(appPath), appBytes,
			timeout, insecure)
	},
}
...
// printEnvResult prints the environment a command just produced (create --wait, resume).
// CLI-041: -o json gets the full environment record - the same camelCase schema as
// `env get -o json`, including id - and never envRow, which is a DISPLAY struct with only
// header: tags, so encoding/json would emit Go field names and no id at all. Table and CSV
// keep the compact row they always had. Do NOT fix this by adding json: tags to envRow:
// that would make a display struct a supported machine surface.
func printEnvResult(p *output.Printer, env environment) error {
	if p.Format == output.FormatJSON {
		return p.Print(env)
	}
	return p.Print(toEnvRow(env))
}
...
func printEnvDetails(p *output.Printer, e environment) error {
	...
	// SEC-039: USERNAME stays (tier-1 standing-visible). The password and the web-service
	// access key are no longer in this response at all - `bcdock env credentials <env>`
	// reveals them, and records that it did.
...
func init() {
	...
	envCreateCmd.Flags().String("manifest", "",
		"Path to a bcdock.manifest.yaml. Implies --wait, because apps need a running environment. "+
			"Precedence: an explicitly passed flag beats the manifest, which beats the built-in default")
	envCreateCmd.Flags().Bool("insecure", false,
		"Skip TLS verification when publishing manifest apps to the dev endpoint")
	...
	// Query flags (DEV-088). No --method flag exists, deliberately: read-only is a
	// property of the code, so a write attempt fails as an unknown flag before any call.
	envQueryCmd.Flags().String("company", "", "BC company to scope the query to (wraps the path in Company('...'))")
	envQueryCmd.Flags().String("tenant", "default", "BC tenant to query")
	envQueryCmd.Flags().String("filter", "", "OData $filter expression")
	envQueryCmd.Flags().String("select", "", "OData $select list (comma-separated field names)")
	envQueryCmd.Flags().Int("top", 0, "OData $top row limit (0 = server default)")
	envQueryCmd.Flags().Duration("timeout", 60*time.Second, "Max time to wait for the OData response")
	envQueryCmd.Flags().Bool("insecure", false,
		"Skip TLS verification for the OData endpoint (use for self-signed local certs only)")
	...
	envCmd.AddCommand(envCreateCmd, envListCmd, envGetCmd, envCredentialsCmd, envDeleteCmd, envLogsCmd, envUsageCmd, envOpenCmd, envDownloadSymbolsCmd, envPublishCmd, envQueryCmd, envLaunchJsonCmd, envHibernateCmd, envResumeCmd, envWaitCmd)
