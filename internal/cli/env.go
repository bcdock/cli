...
Config discovery: BC versions with a pre-built VM image (~2 min provisioning)
are called "fast configs". Versions without one trigger a ~78 min image build
on first use - usually not what you want. Always prefer fast configs unless
you explicitly need a specific version.
...
Examples:
  bcdock env create                                           # interactive picker
...
Exit codes:
  ...
  124 timeout elapsed without the env reaching any requested state
...
func pickFastConfig(ctx context.Context, c *client.Client) (*fastConfigChoice, error) {
	...
	fmt.Fprintln(os.Stderr, "Fast configs (pre-built images, ~2 min provisioning):")
