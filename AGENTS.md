...
- Cursor users: `.cursorrules` points here. Copilot users: `.github/copilot-instructions.md` does the same.
...
|---|---|---|
| `0` | Success | Continue |
| `1` | Generic error: network, a usage error (bad flag, missing arg), a conflict or invalid state (409/400), a failed provisioning, or a `--wait` that timed out | Read stderr (or the JSON `error` code, below); surface to user |
| `3` | Not authenticated or not allowed (401/403) | Run `bcdock auth whoami` to confirm; re-auth, or you may need a different role or API key scope |
| `4` | Rate limited (429) | Back off and retry |
| `5` | Resource not found (404) | Confirm the name/id; list to see what's there |
| `124` | `env wait` did not see a requested status (timed out, or reached another terminal status) | Check `bcdock env get`; increase `--timeout`, or hand off |

No other code is emitted today (`2`, `6` and `10` are not used). With `-o json`, a failure also writes one JSON object to stderr - `{"error":"<code>","message":"...","exitCode":N}` plus `"status"` for an API error - so branch on `error` rather than parsing text.

### 4.1 Recovery catalogue

When the API returns `{"code": "<x>", ...}`, your next call should be:

| Code | Cause | Recovery |
|---|---|---|
| `unauthorized` | Token missing or invalid | `bcdock auth whoami` to confirm; `bcdock auth login` to re-auth |
| `forbidden` | Token valid but lacks scope for this resource | Surface to user; they may need a different API key scope or role |
| `not_found` | Resource id / name doesn't exist | List the resource type to confirm (e.g. `bcdock env list`); fix the id |
| `invalid_input` | A field failed validation | Read `details.field` from the JSON response; fix that field and retry |
| `missing_required_field` | A required field is absent | Read `details.field`; add it and retry |
| `invalid_state` | Resource is not in the state this operation requires | `bcdock <resource> get <id> --output json` to check current state |
| `conflict` | Resource already exists, or operation conflicts with in-flight op | The resource may already be in the desired state; verify before retrying |
| `quota_exceeded` | Plan's environment quota hit | `bcdock env list` then `bcdock env delete <unused>`; or upgrade plan |
| `trial_exhausted` | Trial period ended | Subscribe via portal; `bcdock me` shows plan state |
| `stored_cap_exceeded` | Plan's stored-environment cap hit | Delete a hibernated env to free a slot; or upgrade plan |
| `company_suspended` | Account suspended (billing, ToS, or admin action) | Contact support; do NOT retry - retries will not change the state |
| `subscription_required` | Operation requires an active paid subscription | Subscribe via portal; surface to user |
| `rate_limited` | Hit per-token rate limit | Back off and retry; `bcdock env wait` already paces correctly |
| `internal_error` | Server-side bug | Retry ONCE with backoff; if it persists, surface and stop |
| `service_unavailable` | Downstream dependency unhealthy | Retry with exponential backoff (5s, 15s, 45s, then stop) |

For non-canonical codes (anything not in this table), treat as a generic
error: surface to the user and stop.
...
```bash
$ bcdock env create --name dev --version 99.9 --output json
{"error":"version not in catalogue","code":"invalid_input","details":{"field":"version"}}
$ echo $?
1
```

```bash
$ bcdock env create --name dev --version 26.0
error: quota_exceeded: Plan environment quota reached (3 / 3).
       Free a slot: bcdock env list && bcdock env delete <unused>
$ echo $?
1
```
